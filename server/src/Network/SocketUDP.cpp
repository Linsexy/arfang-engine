...
Net::SocketUDP::SocketUDP(uint16_t const &port) : _currentClient(1)
{
#ifdef WIN32
    WSADATA wsa;
...
ssize_t     Net::SocketUDP::sendMsgByID(size_t id, const void *buffer, size_t length)
{
    try
    {
        auto const &client = _clients.at(id);
        return (sendMsg(client.first, client.second, buffer, length));
...
ssize_t     Net::SocketUDP::recvMsg(void *buffer, size_t length)
{
    ...
    auto const &client = std::find_if(_clients.begin(), _clients.end(),
                                      [from](auto const &elem){ return elem.second.first ==
                                                                       std::string(inet_ntoa(from.sin_addr)); });
    if (client == _clients.end())
        _clients[_currentClient++] = std::make_pair(std::string(inet_ntoa(from.sin_addr)), ntohs(from.sin_port));
